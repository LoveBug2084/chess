 ...
 *
 *  Loaded only when the page is opened with ?test=enpassant in the URL, so
...
/* ------------------------------------------------------------------ *
 *  CASTLE TEST POSITION
 *
 *  Sets up South king at G1 with both rooks (D1 and K1) for testing
 *  both queen-side and king-side castling. Only South has pieces.
 * ------------------------------------------------------------------ */
function loadCastleTest() {
  ...
  // South rook (queen-side) at D-1 (label row 1) -> internal row 13
  // Column D -> internal col = 3
  placePiece(13, 3, 'rook', colSouth, 'south');
  boardState['13,3'].hasMoved = false;

  // South rook (king-side) at K-1 (label row 1) -> internal row 13
  // Column K -> internal col = 10
  placePiece(13, 10, 'rook', colSouth, 'south');
  boardState['13,10'].hasMoved = false;
